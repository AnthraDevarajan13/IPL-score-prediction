...
training_set = training_set[["current runs", "Wickets", "Balls Remaining", "Total runs", "diff", "Home", "Toss",  "batting_team", "bowling_team"]]
...
for row in range(len(testing_set)):
    new_prediction = int(rf.predict(sc.transform(np.array([testing_set.iloc[row]])))[0])
